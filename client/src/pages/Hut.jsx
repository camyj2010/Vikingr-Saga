...
import ava from '../img/IconAvatar.png'
    ...
    <span className="uploaded_file_label">{uploadedFile}</span>
        ...
        <div className='avatar_hut'>
          <Canvas className='canvasAvatar' id="canvas" camera={[0, 0, 0]} >
            ...
            <Avatar type={userInfo?.avatar} urlAvatar={"/avatars/" + userInfo?.avatar + ".glb"} position={[0, -1, 3.4]} rotation={[0, Math.PI / 8, 0]} scale={1} />
          </Canvas>
          <button onClick={() => setPopUpAvatar(!popUpAvatar)} className="avatar_button"><img className='icon_close_modal' src={ava} /></button>
        </div>
        ...
        {popUpAvatar ? <ModalAvatar show={popUpAvatar} change={setPopUpAvatar} setAvatarChanged={setAvatarChanged} /> : ''}
